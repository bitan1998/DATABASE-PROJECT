...
def searchData(StdId="",Firstname="",Surname="",Dob="",Emailid="",Subject="",Address="",Mobile="",RfId=""):
	conn=sqlite3.connect("student.db")
	cur=conn.cursor()
	cur.execute("SELECT * FROM student WHERE StdId=? OR Firstname=? OR Surname=? OR Dob=? OR Emailid=? OR Address=? OR Mobile=?OR RfId=?",\
